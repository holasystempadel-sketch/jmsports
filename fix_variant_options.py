...
def jim(ep, retries=4):
    for a in range(retries):
        try:
            r = jsess.get(f'{JIM_BASE}/{ep}', timeout=45)
...
def build_jim_index():
    ...
    index = {}; lock = threading.Lock(); cnt = [0]
    ...
    def work(pid):
        ...
        with lock:
            ...
            if cnt[0] % 300 == 0:
                print(f'  index {cnt[0]}/{len(ids)} | {len(index)} EANs', flush=True)

    with ThreadPoolExecutor(max_workers=10) as ex:
        list(ex.map(work, ids))
    print(f'Índex Jim llest: {len(index)} EANs', flush=True)
    return index
